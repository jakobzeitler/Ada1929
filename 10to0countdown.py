...
def button_callback(xcoord, ycoord, edge):
    ...
    if edge == NeoTrellis.EDGE_RISING:
        ...
        if button not in buttons:
            ...
            note_level = base_note + 12*y + major_scale_note_for(x)
...
def major_scale_note_for(x):
    return major_scale[x]


def send_grid_MIDI(grid):
    for x in range(width):
        for y in range(height):
            if(grid[x,y] == ON):
                note_level = base_note + 12*y + major_scale_note_for(x)
...
characters = {}
characters['0'] = [0, 60, 102, 66, 66, 102, 60, 0]
characters['1'] = [0, 24, 56, 88, 24, 24, 126, 0]
characters['2'] = [0, 60, 110, 70, 12, 24, 126, 0]
characters['3'] = [0, 60, 70, 6, 30, 70, 60, 0]
characters['4'] = [0, 24, 32, 72, 126, 8, 8, 0]
characters['5'] = [0, 126, 64, 124, 2, 66, 60, 0]
characters['6'] = [0, 60, 66, 64, 124, 66, 60, 0]
characters['7'] = [0, 126, 6, 12, 62, 24, 48, 0]
characters['8'] = [0, 60, 66, 66, 60, 66, 60, 0]
characters['9'] = [0, 60, 66, 62, 2, 66, 60, 0]

def draw_character(bitmap):
    for dot in range(raw_width*raw_height):
        y = dot % raw_height
        x = int((dot - y)/raw_width)
        print(x,y)
        trellis.color(x, y, OFF_COLOR)
    ...
    for x, row in enumerate(bitmap):
        row = bitmap[-x-1]
        num_bits = 8
        bits = [(row >> bit) & 1 for bit in range(num_bits - 1, -1, -1)]
        print(bits)
        for y, bit in enumerate(bits):
            if bit:
                ...
                y_cor = y# - raw_height
                print(x_cor,y_cor)
                trellis.color(x_cor, y_cor, PURPLE)
    time.sleep(1)

def show_character(character):
    if character in characters.keys():
        draw_character(characters[character])
